...
class Account extends Model{
	...
	public function ckeckRefExists($ref){
		$params = [
			'ref' => $ref,
		];
		return $this->db->column('SELECT id FROM accounts WHERE login = :ref', $params);
	}
	...
	public function register($post){
		...
		if($post['ref'] == 'none'){
			...
			$ref = $this->ckeckRefExists($post['ref']);
			if(!$ref){
				$ref = 0;
			}
		}
